...
import stijgmachine.jti1a1.nl.view.WaveView;
...
public class WaterSupplyGame extends MiniGameLogic {
        ...
        public WaterSupplyGame() {
                ...
                this.size = new Dimension((Main.resX / 8)+5 ,(Main.resY / 8));
                addFloatingPipes();
                addWaves();
        }

        private void addWaves() {
                for(int wi = 2; wi < 21; wi++){
                        waveView.add(new WaveView(new Point2D.Double(80 * wi, 920+wi+2)));
                }
        }

        private void addFloatingPipes() {
                floatingPipes.add(floatingPipe = new GamePipes(new Point2D.Double(175, 880), 0, size));
                floatingPipes.add(floatingPipe = new GamePipes(new Point2D.Double(400, 870), 0, size));
                floatingPipes.add(floatingPipe = new GamePipes(new Point2D.Double(665, 880), 0, size));
                ...
                floatingPipes.add(floatingPipe = new GamePipes(new Point2D.Double(900, 890),0,size));
        }
...
//                        System.exit(0);
        ...
        public void tick() {
                ...
                if (go){
                        ((GrappleCursor)gameObjects.get(0)).update((int)getCursorLocation().getX(),(int)getCursorLocation().getY());
                        map();
                }
        }
                        ...
                        if (gameObject.getClass() == GamePipes.class){
                                ...
                                if(mapStart){
                                        if ( ((GamePipes) gameObject).getLocation().getY() < startLocation.getY() &&         ((GamePipes) gameObject).getLocation().getY() < startLocation.getY() + size.getHeight()
                                                ...
                                                musicPlayer.playSound("./sound/snap.wav", 0);
                                                add = true;
                                                mapStart = false;
                                        }
                                }
